...
import { Board, BoardBodyUpdate } from "@/interface/board.interface"
import axios, { AxiosRequestConfig } from "axios"
import moment from "moment-timezone"
moment.tz.setDefault("Asia/Bangkok")
moment.locale("th")

const BoardEditApi = async (PREFIX: string, body: BoardBodyUpdate): Promise<GatewayResponseData<Board>> => {
    try {
        const responseData = await BoardEditService(PREFIX, body)
        return {
            isError: false,
...
const BoardEditService = async (PREFIX: string, body: BoardBodyUpdate): Promise<GatewayResponseData<Board>> => {
    try {
        const options: AxiosRequestConfig = {
            method: "POST",
            url: process.env.API_ENDPOINT_URL_BOARD_UPDATE as string,
            headers: {
        ...
        console.log(PREFIX, " BoardEditService response Data : ", JSON.stringify(response.data))
        ...
        if (axios.isAxiosError(error)) {
            ...
            console.error(PREFIX, " BoardEditService Error response Data : ", errorResponse)
