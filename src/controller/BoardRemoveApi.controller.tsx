...
import { Board, BoardBodyRemove } from "@/interface/board.interface"
import axios, { AxiosRequestConfig } from "axios"
import moment from "moment-timezone"
moment.tz.setDefault("Asia/Bangkok")
moment.locale("th")

const BoardRemoveApi = async (PREFIX: string, body: BoardBodyRemove): Promise<GatewayResponseData<Board>> => {
    try {
        const responseData = await BoardRemoveService(PREFIX, body)
        return {
            isError: false,
...
export default BoardRemoveApi

const BoardRemoveService = async (PREFIX: string, body: BoardBodyRemove): Promise<GatewayResponseData<Board>> => {
    try {
        const options: AxiosRequestConfig = {
            method: "POST",
            url: process.env.API_ENDPOINT_URL_BOARD_DELETE,
            headers: {
